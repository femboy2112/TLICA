...
## The Matrix, as a picture

The paper uses Neo's arc as an *illustration that earns no evidential weight* — and says so
plainly. Neo trains up many locally powerful capacities, fights well, survives — yet fails the
first rooftop jump, and his local wins never touch the fact that the Agents are *generated by* the
surrounding system. Then the death / Trinity-kiss sequence marks not an added fact but a change of
relation to the field, and immediately after, Neo **sees the code**: bullets, hallway, and Agent
stop being independent obstacles and become expressions in one generating representation. The
shift the paper cares about is the one line:

$$\text{How do I dodge this bullet?} \;\longrightarrow\; \text{What is a bullet in the generating representation?}$$

He doesn't finally memorize every move; he learns *what a move is relative to the field.* A clean
picture of nucleation — and nothing more than a picture.
