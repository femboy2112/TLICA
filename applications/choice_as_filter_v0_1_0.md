...
**Status:** Application paper — a conservative fidelity refinement of *Free Will* and *Agency Architecture*, v0.1.0 (first draft)  
**Date:** 2026-08-29 (integrated to `main`; authored on branch `research/choice-as-filter-perceptual-momentum-2026-08-29`)  
...
- [`applications/free_will_v0_3_0.md`](free_will_v0_3_0.md), especially the six conditions of free choice;
- [`applications/agency_architecture_v0_3_0.md`](agency_architecture_v0_3_0.md), especially live-option construction, operator-bundle selection, order-indexed willing, decision-events, timing-marker non-identity, and the variation taxonomy;
- [`applications/this_is_water_truth_respecting_choice_v0_1_0.md`](this_is_water_truth_respecting_choice_v0_1_0.md), especially micro-periagoge, selection closure, and compiled transport.
...
$$
\boxed{
\mathrm{Im}(\pi^m_t)=\mathfrak F^m_t.
}
$$
...
\mathcal U^m_t
\subseteq
\mathrm{Cl}(\mathsf{Tools}^m_t;\mathsf A^{+,m}_t)
$$
...
\mathsf{Foc}^m_t(f)
=
\mathrm{Top}_{M_m(t)}\big(\omega^{f,m}_t\big),
$$
...
- $\mathrm{VEI}>0$: the chosen filter increased net integration relative to baseline;
- $\mathrm{VEI}<0$: it increased net weakening, inhibition, separation, or non-integration relative to baseline;
- $\mathrm{VEI}=0$: no detectable differential update.
...
- low $\mathrm{PM}$: a small admissible filter deformation can make $B$ dominant;
- high $\mathrm{PM}$: a strong, sustained, recursive, or specially structured deformation is required;
...
$\mathrm{PM}$ summarizes the joint resistance already generated by:
...
| A filter space can conservatively parameterize existing self-directed focus without adding reachable states | **DISCLOSED conditional on** $\mathrm{Im}(\pi_t)=\mathfrak F_t$ | Formal representation result; the image condition must be preserved in any implementation. |
