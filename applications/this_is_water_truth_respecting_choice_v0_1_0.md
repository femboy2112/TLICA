...
```mermaid
flowchart TD
    A["Inherited default route"] --> B["Timely Mode B objectification"]
    B --> C["Source-marked live alternative"]
    C --> D["Truth-respecting comparative selection"]
    D --> E["Changed orientation or action"]
    E --> F["Optional consolidation"]
    B -. "too late" .-> A
    C -. "not live or captured" .-> A
```
