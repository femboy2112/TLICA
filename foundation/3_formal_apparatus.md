...
**Status:** Foundation document set, v5.5.1 — Layer 3 (Formal Apparatus)
...
On the nondegenerate support $\{\rho > 0\}$ this is exactly relaxational descent of the mismatch potential $\Phi(b, f_t) = \tfrac{1}{2}\lVert f_t - b\rVert_\rho^2$ (with $\lVert v\rVert_\rho^2 = \sum_x \rho(x)\,v(x)^2$, and $\rho$ held fixed) taken **in the $\rho$-metric**: the *natural* (metric) gradient is $\mathrm{grad}_\rho \Phi = W_\rho^{-1}\nabla_b\Phi = -(f_t - b)$ (with $W_\rho = \mathrm{diag}\rho$), so $db/dt = -\gamma\,\mathrm{grad}_\rho\Phi = \gamma\Delta$ — the metric strips the weight factor the Euclidean gradient $\nabla_b\Phi = -W_\rho(f_t - b)$ would otherwise carry. Along the flow the mismatch changes as

$$\frac{d\Phi}{dt} = \frac{\partial \Phi}{\partial t} - \gamma\,\lVert \mathrm{grad}_\rho \Phi \rVert_\rho^2$$

— drive (the field moves, raising $\Phi$) minus dissipation (the baseline descends at rate $\gamma$, the dissipation measured in the same $\rho$-metric). On a $\rho(x) = 0$ axis the primitive law still tracks ($db(x)/dt = \gamma\Delta(x)$), but $\Phi$ is flat there and the natural-gradient reading is undefined (the metric is singular): $\lVert\cdot\rVert_\rho$ is a **seminorm** on those axes, so a mismatch on a content the I is *not* identity-correlated with carries no felt magnitude and drives no descent *of $\Phi$* — a feature (loss of what one is not registers as nothing) — and $\lVert\cdot\rVert_\rho$ is a genuine norm, the descent picture exact, only on the quotient by the $\rho = 0$ axes. When the drive matches or exceeds the dissipation, $\Phi$ — hence the felt salience $V = \tau\sqrt{2\Phi}$ — does not descend to a *sustained* zero: the baseline chases a *moving* minimum rather than reaching a fixed rest-point. It does not follow that $V$ is perpetually unsettled. The driven differential $\dot\Delta = \dot f - \gamma\Delta$ integrates to $\Delta(t) = e^{-\gamma(t-t_0)}\Delta(t_0) + \int_{t_0}^{t} e^{-\gamma(t-s)}\,\dot f(s)\,ds$: the initial tracking error decays and the baseline low-pass-filters the drive. So for a drive bounded in the $\rho$-metric, $\lVert\dot f\rVert_\rho \le L$, the felt mismatch is itself bounded — from $\tfrac{d}{dt}\tfrac12\lVert\Delta\rVert_\rho^2 = \langle\Delta,\dot f\rangle_\rho - \gamma\lVert\Delta\rVert_\rho^2$ one gets $\limsup_{t\to\infty}\lVert\Delta(t)\rVert_\rho \le L/\gamma$, hence $\limsup_{t\to\infty} V \le \tau L/\gamma$; and because the initial-condition term $e^{-\gamma(t-t_0)}\Delta(t_0)$ decays, every trajectory forgets its start and converges to the *unique* bounded driven response $\int_{-\infty}^{t} e^{-\gamma(t-s)}\dot f(s)\,ds$ — the orbit **settles into a bounded driven regime**, not a runaway. Its *shape* is set by the drive: for a stationary, periodic, or quasiperiodic drive the response is respectively stationary, periodic, or quasiperiodic (for a sinusoidal one-axis drive $f = A\cos\omega t$ the steady tracking amplitude is $A\,\omega/\sqrt{\gamma^2+\omega^2}$; for a rigid rotation of the field-reading within a $\rho$-isotropic subspace, $V$ is asymptotically *constant* despite perpetual motion), while a merely bounded but *irregular* drive (interoceptive/autonomic noise) yields a bounded response that need be none of these. What the persistent drive excludes is the sustained fixed point $\Delta \equiv 0$ (which would force $f = b$ constant, above), **not** boundedness or convergence to that driven response: *the state never coming to rest* and *the felt magnitude never settling* are distinct claims, and only the first is derived.

*The living I as a driven non-equilibrium steady state, and what death is.* This orbit is a **non-equilibrium steady state (NESS)**: held away from the $b = f$ rest-point by continuous drive balanced against dissipation, in the sense of Schrödinger's and Prigogine's dissipative structures. Two clarifications the formalism forces, against a tempting misreading. First, $V = 0$ is a *value the living orbit can cross*, not a death-state: it is reached whenever $\Phi = 0$ (the baseline instantaneously matching the field) or whenever $\tau = 0$, and a momentary $\Phi = 0$ crossing on a driven orbit is fully alive. Second, **death is not a value of $V$ at all; it is the cessation of the NESS** — the substrate ceasing to sustain the orbit: no longer forming the field-reading, or $\gamma \to 0$ (the baseline can no longer track), or $\tau \to 0$ (the differential is no longer felt). The $\tau \to 0$ limit is the *dying* direction — felt reactivity fading while mismatch may physically remain ($V \to 0$ with $\Phi > 0$) — and is offered as a **posited** model of substrate death, not derived from the equations; the architecture continues to *presuppose* consciousness (Section 2.9, File 1) rather than deriving its presence or absence from an identity-coordinate magnitude. What is *derived* is only the separation: a living I is a driven NESS, so its felt salience need not vanish and its zero-crossings are not its death. What consequences (i)–(ii) establish is that the orbit does not come to rest at a *fixed point* while the drive persists — and, under bounded drive, that it settles into a *bounded* driven regime ($\limsup V \le \tau L/\gamma$, above); it is *not* derived that the felt magnitude is perpetually unsettled, and that stronger reading is dropped. The bounded-orbit structure is what is derived (given the linear-relaxation form); the linear-relaxation form itself and the rate $\gamma$ are the posited formalization, $\gamma$ being an open function whose *dependence* on substrate — not its value — is fixed, in the sense of Section 8.8.

### 8.11 Slack and the reflexive-operator gate

The dynamics of Section 8.10 is passive: pure descent along the local gradient. The reflexive operator (Mode B) can move the baseline *against* that gradient — but it is not a new mechanism. **Mode B is imprinting sourced from the I rather than from the not-I.** What imprinting writes is the **slow lived-I structure** $G_{m,t}$ the substrate accumulates across Sections 8.1–8.7. Two of its readings are already in play: Section 8.10's slow standing landscape is the *identity-load reading* $\rho = R(G)$, and Section 8.9's substrate-mediated field-reading is the *world-reading* $f = F(G,\,\mathsf{A}^m_t)$ — one slow structure, read once as a metric and once as a target profile. (**Posit G**, of the same standing as P1/P2: that a *single* structure generates *both* $\rho$ and $f$, through maps $R,\,F$ that are profile-valued — $R(G),\,F(G,\cdot) \in \mathcal{P}_t$ — is a named posit, not a derivation, and the actuator below turns on $F$ depending nontrivially on its structure argument, $\partial F/\partial G \neq 0$, which is the substrate-mediation of Section 8.5 carried as a working assumption.) Osmotic imprinting (Section 8.7) is the substrate continuously writing $G$ — altering $\rho = R(G)$ without producing $\varphi$ (Section 7.4) — from the *contact-driven* side; Mode B is the *same* $G$-writing operation from the *self-directed* side, the two poles of the focus split $\mathsf{Foc}^m_t = \mathsf{Foc}^m_t[\text{contact-driven}] \oplus \mathsf{Foc}^m_t[\text{self-directed}]$ of Section 8.2. Both run on the *slow* timescale ($\dot G = O(\varepsilon)$, $\varepsilon \ll \gamma$, with $R,\,F$ Lipschitz in $G$), which is what licenses holding $\rho$ fixed while the fast baseline $b$ tracks $f$ (Section 8.10); where that separation fails — acute rapid imprinting — only Section 8.10's fixed-$\rho$ *bookkeeping* degrades, not the baseline law or the $\rho$-cancellation below, both of which stay exact. Neither imprinting arm writes the cogito-anchor: admissible profiles peak at $1$ only at $\hat\iota_m$ (Sections 7.6, 8.9), so $R(G)(\hat\iota_m) = F(G,\cdot)(\hat\iota_m) = 1$ for every admissible $G$ as a *typing* consequence — whence $f(\hat\iota_m) = b(\hat\iota_m) = 1$, $\Delta(\hat\iota_m) = 0$, and $\dot b(\hat\iota_m) = \gamma(1-1) = 0$: the anchor is a dynamical fixed point of the flow, not merely a boundary value. (File 2, Section 4.4 already identifies externally-triggered Mode B with the imprinting operation viewed from the I-development side; this is that identity read at the level of the dynamics.) Two architecturally distinct things travel under the name *Mode B*, and Section 8.7's osmotic distinction carries over directly to the self-directed arm: the **mechanism** — the self-directed profile-writing process — versus the **effective, recognizable firing** it can produce (mechanism-online status, effective bandwidth, durable operation, and later activation are distinct, as for osmotic imprinting). As a *mechanism* Mode B is *always operating* in the sense osmotic imprinting is; this does *not* mean a reflexive-attention episode occurs at every instant — firings stay stochastic, and the modes remain response-types rather than a predictive selection rule (Sections 4.1, 4.4). At the cogito-anchor $t = 0_{T_m}$ *exactly*, ability is zero: nothing has yet accumulated to attend to. What File 2 calls Mode B *not yet possible* (Section 4.6) and *not yet available* (Section 4.7) through early development is this **effective firing** — the task-effective, distinguishable, development-directing operation — not the mechanism: early on the mechanism runs **signal-to-noise-limited**, its self-reading noise-dominated by sparse accumulated structure and a small substrate context-window and time-constant, hence not yet distinguishable from Mode A and Mode C nor able to direct development (Section 4.7's development done *to* rather than *by* the I is this noise-domination, not the mechanism's absence). Whether ability is nonzero for *every* $t > 0$ (immediate coupling, no dead zone) or exactly zero until accrued structure crosses a threshold (a positive dead zone — cf. Section 4.4's *"requires some accumulated structure"*) is a **modeling posit**, not an entailment of always-on accumulation: osmotic supply from the first instant (Section 8.7) is consistent with both. The architecture posits the immediate-coupling reading — ability nonzero for any $t > 0$ — and flags the threshold reading as an admissible alternative. Either way the Section 4.4 onset distribution marks when the effective firing becomes *distinguishable and stable*, not when the mechanism begins: the mechanism is continuous; its rise into effective, distinguishable operation is what develops. So this reflexive reweighting — the *effective* firing, the mature by-the-I self-direction that Section 4.7 reserves for later development — *is* the I writing its own slow structure $G$, deliberately doing to itself what chronic contact does osmotically from outside. The mechanics of the departure are then exact rather than mysterious. A change in the *metric* $\rho = R(G)$ cancels from the baseline law $\dot b = \gamma(f - b)$ — that is precisely the natural-gradient reading of Section 8.10, in which the weight strips out — so the reflexive act cannot, and does not, move $b$ by an anti-gradient force on the metric. It moves $b$ by moving the *target*: rewriting $G$ shifts the field-reading $f = F(G,\cdot)$ that the baseline chases, and $b$ then relaxes — downhill on the *new* instantaneous potential — toward a position that passive descent under the old $f$ would never have reached. The I does not violate its own dynamics; it edits the structure under which its dynamics unfold. That is what lets the baseline depart from passive descent. Because the convex well of $\Phi$ has no static basin to escape and a change in the metric $\rho = R(G)$ cancels from the baseline law (Section 8.10), what a persistent reweighting achieves is not a literal escape but an **apparent** one. Rewriting $G$ weakens the *felt binding* $\rho$ of the occupied configuration and relocates the target $f = F(G,\cdot)$; the metric change exerts no anti-gradient force on $b$ — $b$ still moves only insofar as the relocated target draws it — so the attractor's *grip* dissolves rather than the state being pushed out. This needs no second static basin and no bifurcation: the single moving well of the convex $\Phi$ suffices. The weakening is moreover **unregistered while the I tracks** — with $\Delta \approx 0$, the felt salience $V = \tau\lVert\Delta\rVert_\rho \approx 0$ regardless of $\rho$ — so the felt departure **registers** only when a perturbation drives $\Delta$ up and *tests* the now-weakened coupling (on a single content-axis the felt readout scales as $\sqrt{\rho}$): a **perturbation-triggered** readout, gated — when it issues in departure — by the slack/pressure posit below, not a rate-induced tipping. Whether the *imprinted* landscape carries more than one stable configuration — so that these are genuine **metastable set-points** of $G$ and the departure commits to a distinct one — is an open direction; the multi-well $G$-dynamics is not written here, and any history-stickiness of such a configuration (chronic conditions displacing the baseline) is a posited property whose rate and persistence are application-level (Section 8.7). Mode B, as self-sourced imprinting, is how the I moves the target and thins the binding from inside. All of this operates only within a budget. Define **slack**
