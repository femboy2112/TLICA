...
**Status:** **FROZEN v1.0.2** — author-approved publication text; preprint freeze (2026-09-20), tagged `moreish-v1.0.2`. Patch over v1.0.1 (`moreish-v1.0.1`): removed the internal "Reference status" bookkeeping note from after the reference list — its content is retained in the dossier's source/novelty audit — for a clean deposited artifact. (v1.0.1 hedged three overclaim sentences and closed the two closest novelty flanks with Crossref-verified citations: Alchourrón, Gärdenfors, & Makinson 1985; Taber & Lodge 2006.) Descends from canonical application v0.2.9. This is the frozen public-preprint master; the canonical `applications/` paper and the double-blind cut are regenerated from it. Downstream steps remaining: the Zenodo/PhilArchive deposit and the P&CS journal submission (the editable-source journal build is done, kept local).
**Date:** 2026-09-20.
