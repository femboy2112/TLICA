...
| **Slack** (§5) | "This is particularly important under low slack" — expensive model-predictive control is unaffordable when slack is low. | **Zero occurrences in the frozen foundation.** "Slack" is a pinned glossary term and its home is application/research-tier ([cold_frame](../../applications/cold_frame_v0_4_3.md); geometry dossier). The glossary pin is the *will-room* sense (residual latitude the unchosen parameters leave for willing); the essay uses a *cognitive-load / planning-budget* sense (closer to cold_frame's load-driven thinning). | **Leans on an OPEN construct.** Not foundational; the two senses are adjacent but not identical; and slack is **not operationalized** in the current dynamical working model. **Loudest flag in the dossier.** |

**Net:** three of four anchors are faithful (one to the foundation, two to research-tier
siblings); the fourth (slack) is the essay's weak load-bearing point and must be labeled,
not leaned on. No anchor is fabricated or misattributed to the freeze. The §5 sentence
"TLICA supplies a useful structural asymmetry" is sound **only** for the Mode-B half; the
slack half of §5 rests on the open construct above.
...
|---|---|---|---|
| M1 | Maximizing the integrand `u(x,a)` locally does not maximize the integral `J` when the state is coupled (`x_{t+1}=F(x_t,a_t)`). (§4 boxed) | **DERIVED** — standard result (Bellman); **OBSERVED** in the toy (coupled greedy J=12 < optimal 30). | A counterexample to Bellman (there is none); or a coding error in the demo (cross-checked by brute force). |
...
| X2 | The §5 low-slack argument. | **UNVERIFIED dependency** — leans on an open, non-foundational, non-operationalized construct (§1). | Operationalizing slack; or rephrasing §5 to rest only on the Mode-B asymmetry. |
