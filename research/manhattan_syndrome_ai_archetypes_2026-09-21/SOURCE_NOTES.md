...
## S10 — X feedback-loop clause (added v0.4.0, from the 2026-09-21 audit; UNVERIFIED this session)

The 2026-09-21 audit reports that current X documentation states that **when users interact with Grok-powered X features (e.g. recommendations), a deployed model may learn from those interactions during normal use even when the ordinary model-training opt-out is enabled.** If accurate, this supplies a *deployed* feedback channel — model output → user interaction → next-round data → model update — i.e. the closing arrow that would upgrade the §6 mechanism from selection-bias / domain-adaptation to a genuine **performative-prediction** loop (Perdomo et al. 2020) for Grok-mediated recommendation surfaces.

**Status: UNVERIFIED by this session.** The claim is carried from the audit, not independently re-fetched here (and the environment's search index has previously returned polluted/simulated results). It must be re-verified against live X/xAI documentation on release day before it enters the manuscript body, and even then stated only at the granularity X actually discloses — never assuming undisclosed mechanics. Source to check: `https://help.x.com/en/using-x/about-grok`. It is deliberately **not** cited in MANUSCRIPT §6, which already states the closed-loop plausibility at the safe granularity.
