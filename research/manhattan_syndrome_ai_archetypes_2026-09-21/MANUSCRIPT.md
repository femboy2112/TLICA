...
**Research draft v0.4.1 — 21 September 2026**
...
Syndrome is shaped by a field that is too socially load-bearing.
...
This is the cleanest experimental route because the true referent distribution is known. A first, **construction-level** instance has been **executed** (experiment protocol §3; results dossier): in a declared synthetic world, a declared selection operator deforms the *fitted* bundle monotonically across all three components, and censored support renders the deformation unrecoverable even under known inverse-propensity weighting. The honest boundary: this instance *fits* a bundle to a selected joint — it does **not** train a learner, so it is a construction-level calibration instance, not yet the developmental-imprint result. The next step, **Probe E2**, trains an actual learner on the synthetic world, freezes it, removes the source, and tests whether the learned representation stays source-conditioned while an inference-time correction fails to repair it. Everything here concerns declared ground truth only and says nothing about any production system.
...
The paper's concern is what happens when the instrument becomes sufficiently load-bearing that its relational geometry is learned as part of the default structure of social meaning.

That is an imprinting-and-geometry problem that can survive correct source attribution.
...
**The gap.** Across this literature the mechanism *family* is established — performative loops, reward hacking, mode collapse, persistent misalignment under steering, implicit bias surviving explicit correction. What is not established, anywhere located, is the specific bundle this paper names: correct source attribution **and** explicit source critique coexisting with a source-deformed operative social-affective *geometry*, plus a synthetic-world demonstration that the deformation survives declared knowledge of the selection operator. A first, **construction-level** instance has now been run (a declared selection operator deforms the fitted bundle; censored support is unrecoverable — see the results dossier), but the version with a **trained learner** (the proposed Probe E2) and the **Grok-specific** version have not. The paper's contribution is to state the bundle precisely enough that they *can* be — and to say plainly that until they are, the Grok-specific claim stays UNVERIFIED.
...
- **Demonstrated (synthetic, construction-level):** in a declared synthetic world (experiment protocol §3), a declared selection operator deforms the *fitted* meaning-geometry bundle (all three components, monotone), and censored source support is unrecoverable even under known inverse-propensity weighting. **Conditional (toy):** whether an inference-time correction repairs a *durable* deformation depends on a stipulated correction-reach parameter — a modeling choice, not a demonstrated property of a trained system. **UNVERIFIED:** that a *trained* learner develops a durable source-conditioned representation surviving source removal while inference-time correction fails (the proposed Probe E2). None of this transfers to Grok or any real model.
