...
*Implemented and executed.* The badge-door world of Section 14 is realized in
[`badge_door_demo.py`](badge_door_demo.py) (standard library, deterministic; raw
outputs in [`badge_door_demo_results.json`](badge_door_demo_results.json)). It runs
the baseline authority path, the eight ablations, and the full 2×2×2×2 factorial,
and its 14 self-checks pass — including the load-bearing one: the identity-placement
axis is inert for access (C-022). Per the boundary stated at the end of this section,
that is a demonstration of internal consistency and of the discriminators, not proof
